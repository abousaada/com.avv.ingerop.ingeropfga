...
sap.ui.define([
    ...
    return Controller.extend("com.avv.ingerop.ingeropfga.ext.controller.BudgetPxSTI", {
        ...
        _createDynamicColumns2: function () {
            ...
            // Separate business numbers by TypeBudg
            var interUfoBusinessNos = [];
            var intraUfoBusinessNos = [];
            var intercoBusinessNos = [];
            ...
            pSTIs.forEach(function (pSTI) {
                ...
                switch (pSTI.TypeBudg) {
                    case "INTERUFO":
                    case "interUFO":
                        if (!interUfoBusinessNos.includes(pSTI.business_no_p)) {
                            interUfoBusinessNos.push(pSTI.business_no_p);
                        }
                        break;
                    case "INTRAUFO":
                    case "intraUFO":
                        if (!intraUfoBusinessNos.includes(pSTI.business_no_p)) {
                            intraUfoBusinessNos.push(pSTI.business_no_p);
                        }
                        break;
                    case "INTERCO":
                    case "interCO":
                        if (!intercoBusinessNos.includes(pSTI.business_no_p)) {
                            intercoBusinessNos.push(pSTI.business_no_p);
                        }
                        break;
                }
            });
            ...
            // Add dynamic Inter UFO columns
            interUfoBusinessNos.forEach(function (businessNo) {
                var column = this._createDynamicColumn(businessNo);
                this._dynamicColumns.push(column);
                treeTable.addColumn(column);
            }.bind(this));
            ...
            // Add dynamic Intra UFO columns
            intraUfoBusinessNos.forEach(function (businessNo) {
                var column = this._createDynamicColumn(businessNo);
                this._dynamicColumns.push(column);
                treeTable.addColumn(column);
            }.bind(this));
            ...
            intercoBusinessNos.forEach(function (businessNo) {
                var column = this._createDynamicColumn(businessNo);
                this._dynamicColumns.push(column);
                treeTable.addColumn(column);
            }.bind(this));
        ...
        _createDynamicColumn2: function (businessNo) {
            return new Column({
                width: "13rem",
                template: new HBox({
                    items: [
        ...
        _createDynamicColumns1: function () {
            var treeTable = this.byId("com.avv.ingerop.ingeropfga::sap.suite.ui.generic.template.ObjectPage.view.Details::ZC_FGASet--budgets--BudgetPxSTITreeTable");
            if (!treeTable) {
                console.error("TreeTable not found");
                return;
        ...
        _addDynamicColumnValues2: function (item, pSTIs) {
            if (!item.dynamicColumns) {
                item.dynamicColumns = {};
            }
